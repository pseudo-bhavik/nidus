...
import { Plus, Trash2, Search, Check, FolderOpen, PanelLeftOpen, AlertCircle, RotateCcw, X, StickyNote, Archive, ArrowRight, Pin } from 'lucide-react';
...
interface CentralMonitorProps {
  ...
  pinnedVaultSections?: VaultSection[];
  onOpenVaultSection?: (sectionId: string) => void;
}
  ...
  return (
    <div className="flex-1 h-full bg-[#f6f6ef] flex flex-col relative select-text">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-3 flex flex-col gap-1.5 font-sans">
        {/* Pinned Vault Shortcuts (Differentiated visually from single bookmarks) */}
        {currentView === 'all' && pinnedVaultSections.length > 0 && !searchQuery && !highPriorityOnly && (
          <div className="mb-4 pb-3 border-b border-neutral-200/80">
            <div className="flex items-center justify-between pb-2 px-0.5">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-extrabold uppercase tracking-wider text-neutral-800 flex items-center gap-1.5">
                  <Archive className="w-3.5 h-3.5 text-[#ff6600]" /> Vault Shortcuts
                </h4>
                <span className="px-2 py-0.2 rounded-full text-[10px] font-extrabold bg-orange-100/70 text-[#ff6600] border border-orange-200">
                  {pinnedVaultSections.length}
                </span>
              </div>
              <span className="text-[10px] text-neutral-400 font-semibold hidden sm:inline">
                Pinned vault collections • Tap to open inside Link Vault
              </span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 pt-1">
              {pinnedVaultSections.map((sec) => {
                const totalLinks = sec.links ? sec.links.length : 0;
                const sampleDomains = sec.links ? sec.links.slice(0, 3).map((l) => l.domain).filter(Boolean) : [];

                return (
                  <div
                    key={sec.id}
                    onClick={() => onOpenVaultSection?.(sec.id)}
                    className="group relative bg-gradient-to-br from-amber-50/40 via-white to-orange-50/20 border-2 border-dashed border-orange-300/80 hover:border-orange-500 rounded-xl p-3 cursor-pointer shadow-2xs hover:shadow-md transition-all flex flex-col justify-between gap-2.5 overflow-hidden"
                  >
                    {/* Top Header */}
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        <div className="w-7 h-7 rounded-lg bg-white border border-orange-200 flex items-center justify-center shrink-0 shadow-2xs text-[#ff6600] group-hover:scale-110 transition-transform">
                          <FolderOpen className="w-4 h-4 text-[#ff6600]" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <h5 className="font-extrabold text-xs sm:text-sm text-neutral-900 truncate group-hover:text-[#ff6600] transition-colors">
                            {sec.title}
                          </h5>
                          <span className="text-[10px] font-semibold text-neutral-500">
                            {totalLinks} link{totalLinks !== 1 ? 's' : ''} stored
                          </span>
                        </div>
                      </div>

                      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9.5px] font-extrabold bg-[#ff6600] text-white shadow-2xs shrink-0">
                        <Pin className="w-2.5 h-2.5 fill-current" /> VAULT
                      </span>
                    </div>

                    {/* Middle preview domains */}
                    {sampleDomains.length > 0 && (
                      <div className="flex items-center gap-1 flex-wrap pt-0.5">
                        {sampleDomains.map((dom, i) => (
                          <span
                            key={i}
                            className="px-1.5 py-0.5 bg-white/90 border border-neutral-200 rounded text-[9.5px] font-mono font-medium text-neutral-600 truncate max-w-[110px]"
                          >
                            {dom}
                          </span>
                        ))}
                        {totalLinks > 3 && (
                          <span className="text-[9.5px] text-neutral-400 font-bold">
                            +{totalLinks - 3} more
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-1 border-t border-orange-100 text-[11px] font-bold text-[#ff6600]">
                      <span>Open in Link Vault</span>
