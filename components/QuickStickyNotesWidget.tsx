...
import { 
  ...
  ChevronDown, ChevronUp, Copy, Check, Trash2, GripVertical
} from 'lucide-react';
...
export default function QuickStickyNotesWidget({ onOpenFullNotes }: QuickStickyNotesWidgetProps) {
  ...
  // Clear obsolete drag position from localStorage
  useEffect(() => {
    try {
      localStorage.removeItem('nidus_sticky_widget_pos');
    } catch (e) {}
  }, []);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40 select-none flex flex-col items-end animate-fade-in pointer-events-auto floating-sticky-widget">
      {/* Minimized Floating Badge — Sharp Square Corners */}
      {isMinimized ? (
        <div className="flex items-center gap-1 bg-neutral-900 text-white rounded-none shadow-lg p-1 border border-neutral-700">
          <button
            onClick={() => setIsMinimized(false)}
            className="px-2.5 py-1 flex items-center gap-2 text-xs font-bold hover:bg-neutral-800 rounded-none cursor-pointer transition-all duration-200"
        ...
        /* Expanded Floating Sticky Notes Widget Card — Sharp Square Corners & Locked Bottom-Right */
        <div className={`w-[calc(100vw-2rem)] max-w-sm sm:w-96 rounded-none border shadow-xl flex flex-col overflow-hidden transition-all duration-200 ${currentTheme.bg} ${currentTheme.border}`}>
          
          {/* Widget Header — Sharp Corners */}
          <div className={`px-3 py-2 ${currentTheme.header} border-b ${currentTheme.border} flex items-center justify-between`}>
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1 hover:bg-black/10 rounded-none text-neutral-600 cursor-pointer transition-all-custom"
          ...
          {/* Reorderable Tabs Navigation Bar — Sharp Corners */}
              ...
              return (
                ...
                >
                  <button
                    ...
                    className={`px-2 py-1 rounded-none text-[10px] font-bold flex items-center gap-1 cursor-grab active:cursor-grabbing transition-all ${
                      isSelected
                        ? 'bg-white shadow-2xs text-neutral-900 border border-neutral-200 font-extrabold'
            ...
            <div className="p-3.5 flex flex-col justify-between min-h-[140px]">
              <div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1 shrink-0 ml-2">
                    <button
                      ...
                      className="p-1 hover:bg-black/10 rounded-none text-neutral-500 cursor-pointer"
                      title="Change color"
                    >
                    ...
                    <button
                      ...
                      className="p-1 hover:bg-red-100/50 rounded-none text-neutral-500 hover:text-red-600 cursor-pointer"
                      title="Delete note"
                    >
                ...
                {colorPickerNoteId === activeNote.id && (
                  <div className="mb-2 p-1.5 bg-white rounded-none shadow-md border border-neutral-200 flex items-center justify-between animate-fade-in">
                    ...
                    <div className="flex items-center gap-1">
                      {Object.keys(COLOR_STYLES).map((c) => (
                        <button
                          ...
                          className={`w-3.5 h-3.5 rounded-none border border-black/10 transition-transform ${COLOR_STYLES[c].dot} ${
